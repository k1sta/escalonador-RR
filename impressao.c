...
void imprimirTabelaIO(Processo* processos, int numProcessos) {
    ...
    printf("|PID\t|Qntd de IO\t|Tipo de IO\t|Início\t|Tempo de IO\t|Tempo Rest\t|\n");
    printf("|-------------------------------------------------------------------------------|\n");
    for (int i = 0; i < numProcessos; i++) {
	...
	for (int j = 0; j < processos[i].qntdIO; j++){
		printf("|%c\t\t|%d\t|%d\t\t|%d\t\t|\n", processos[i].ios[j].tipo, processos[i].ios[j].inicio, processos[i].ios[j].tempoExec, processos[i].ios[j].tempoExecRestante);
